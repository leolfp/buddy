// #include <Arduino.h>
//  This source is extracted from the following repository
//  source.(examples/Advanced/Bluetooth_with_ESP32A2DP)
...
#define M_PI 3.141592653
  ...
  uint32_t get(size_t index) {
    return (index < FFT_SIZE / 2) ? (uint32_t)sqrtf(_fr[index] * _fr[index] +
                                                    _fi[index] * _fi[index])
                                  : 0u;
  }
};